...
export async function scrapeReservas() {
  ...
  const ordenado = [...detalle].filter((item) => item?.fecha && item.fecha <= hoy).sort((a, b) => b.fecha.localeCompare(a.fecha));
  ...
  if (process.env.BACKFILL === '1') {
    const floor = `${new Date().getUTCFullYear() - 5}-01-01`;
    const historical = detalle.filter((item) => item.fecha >= floor && item.fecha <= hoy && typeof item.valor === 'number' && item.valor > 0).sort((a, b) => a.fecha.localeCompare(b.fecha));
    if (!historical.length) throw new Error(`No hay reservas históricas desde ${floor}.`);
    if (process.env.DRY_RUN === '1') {
      console.log(`✅ prueba backfill reservas: ${historical.length} días · ${historical[0].fecha}–${historical.at(-1).fecha}`);
      return { ...registro, historical };
    }
    const db = getDb();
    const root = db.collection('indicadores').doc('reservas_bcra');
    for (let start = 0; start < historical.length; start += 450) {
      const batch = db.batch();
      historical.slice(start, start + 450).forEach((item) => batch.set(root.collection('historico').doc(item.fecha), {
        idVariable: ID_VARIABLE_RESERVAS,
        descripcion: 'Reservas Internacionales del BCRA',
        fecha: item.fecha,
        valor: item.valor,
        valorMillonesUSD: item.valor,
        unidad: 'Millones de USD',
        fuente: 'api.bcra.gob.ar (API oficial Principales Variables v4.0)',
        scrapedAt: new Date().toISOString(),
      }, { merge: true }));
    ...
    await root.set({ ultimo: registro, backfillDesde: historical[0].fecha, backfillHasta: historical.at(-1).fecha, historialFuente: 'api.bcra.gob.ar · Variable 1 Reservas Internacionales' }, { merge: true });
    console.log(`✅ backfill reservas: ${historical.length} días · ${historical[0].fecha}–${historical.at(-1).fecha}`);
    return { ...registro, historical };
  }
  const db = getDb();
